...
| 19.05.2026 | 26.04/1 | Style improvements. — Changed by: Raul Metsma
...
Since Windows Server 2008 SP2 and Windows Vista SP2 combination it is possible to use Estonian eID cards for Windows domain login. This possibility has been actual since autumn 2008, when the first successful tests were made. This document describes platforms and configurations which enable Windows domain login, achievable using only standard Microsoft and ID-software.
...
*   Client computers must have ID-software installed (the recommended version is 26.4.20.8412).
...
The following chapters describe exact steps to create a working configuration for eID domain logging.

## Domain settings

To prepare Windows domain for eID logging, specific policies must be created for domain controllers and client computers. As prerequisite domain controller must have specific certificate (server authentication, smart card logon) to identify itself and allow smart card logon.
...
To enable domain logging with an eID card, intermediate level certificates (`ESTEID2018`, `ESTEID2025`) must be installed in the NTAuthCertificates container of the domain. This is done with the command `certutil -dspublish -f 'CERTIFICATE NAME' NTAuthCA`. A root-level certificate can also be added to the domain container with the command `certutil -dspublish -f 'CERTIFICATE NAME' RootCA`.

Certificates can be downloaded from <https://www.skidsolutions.eu/resources/certificates/> and <https://repository.eidpki.ee/crt/>. As of today, the following certificates are required:
...
To publish certificates to domain controllers automatically with group policy, it is recommended to modify the Default Domain Controllers or any other OU-level policy for domain controllers. Certificates must be placed into containers according to their type: root certificates into the Trusted Root Certification Authorities container and intermediate certificates into the Intermediate Certification Authorities container.
...
As you can see in the previous illustrations, the certificates become visible in the Trusted Root Certification Authorities and Intermediate Certificate Authorities containers, respectively. With the next policy cycle, these settings will be applied to all domain controllers. Group Policy updates can be forced by running `gpupdate /force` on domain controllers. And as already said, in the same way the required certificates can be published to all other Windows workstations and servers, if necessary.

### Configuring eID card properties in domain

To support eID card domain logging centrally on all client computers, a domain-level policy is used in the following example:[^2]
...
After these changes the policy should look like presented on the following picture:
...
It should be noted that eID cards have two certificates. To log into domain with eID card, the certificate with `Client Authentication` described under EKU must be used.
...
So, it is probably reasonable to follow Microsoft's recommendation and associate the certificate with the user using the `issuer` and `serialnumber` fields. This can be done via the ADSI Edit GUI.
...
The ID-software must be installed on client computers (the recommended version is 26.4.20.8412). In fact, the correct functioning of the eID card minidriver is also sufficient, but the entire ID-software is usually installed as standard.
...
After the configuration takes effect on the client computer, the smart card ![](./img/image14.png) can be selected as the login method in the login window.

![eID card domain login window, waiting for PIN](./img/image15.png)

### Requiring eID card for domain logging

Sometimes it may be desirable to allow users to log in to systems only with an eID card (in other words, prohibiting the use of user passwords). This can be applied to common or specific workstations and/or RDP servers. To apply the requirement, the following policy must be applied to the desired computers:
...
The behavior of a computer or a group of computers when the smart card is removed can also be configured. (This works only when a smart card is used for domain logon.) Options include:
...
---
[^1]: If both the intermediate and root level certificates have already been published in the domain using the previously described method, there is no direct need for republishing. The intermediate certificate can, however, be published by placing it in the domain NTAuthCertificates container and the root certificate with a normal domain policy, as described below. It’s actually a bit confusing, because although in theory Microsoft requires that the CA certificate that issued the card certificate belongs to the NTAuthCertificates container in the domain (check [Guidelines for enabling smart card logon with third-party certification authorities](https://docs.microsoft.com/en-us/troubleshoot/windows-server/windows-security/enabling-smart-card-logon-third-party-certification-authorities)), then in practice the login with the eID card works even if it hasn’t been done and the chain is simply trusted. Anyway, it is recommended to follow Microsoft’s technical requirements when creating eID login configuration.
[^2]: Of course, the policy can be applied from any level or group, for only client computers for example.
...
[^4]: If, as a rule, the issuer is a constant (across the chain), then the serialnumber must be changed for every user. Use this PowerShell function:
